function getDataPoint(data, year, province, level) {
  if (
    data.byYearAndProvince["$" + year] &&
    data.byYearAndProvince["$" + year]["$" + province]
  ) {
...
function drawChoropleth(topoJsonData, data, year, level) {
  ...
  // Legend
  const legendLabels = mapLegendContainer
    .selectAll("text")
...
function updateChoropleth(topoJsonData, data, year, level) {
  const geoJsonData = topojson.feature(
    topoJsonData,
    topoJsonData.objects.states_provinces
  );

  // Map
  const mapPaths = mapContainer.selectAll("path").data(geoJsonData.features);

  mapPaths.exit().remove();
  const mapPathsEnter = mapPaths
    .enter()
    .append("path")
    .attr("class", "province")
    .on("mouseenter", (d) => {
      ...
      mapTooltip.html(
        d.properties.province +
          "<br />APM: " +
          getDataPoint(data, year, d.properties.province, level) +
          "%"
      );
    })
    .on("mousemove", (d) => {
  ...
  // Legend
  const legendBoxes = mapLegendContainer
    .selectAll("rect")
    .data(choroplethLegendColors);

  legendBoxes.exit().remove();
  const legendBoxesEnter = legendBoxes
    .enter()
    .append("rect")
    .attr("y", 6)
    .attr("width", 30)
  ...
  const legendLabelsEnter = legendLabels
    .enter()
    .append("text")
  ...
  // Level picker
  const pickerLabels = pickerContainer
    .selectAll("text")
    .data(["sd", "smp", "sma", "pt"]);

  pickerLabels.exit().remove();
  const pickerLabelsEnter = pickerLabels
    .enter()
    .append("text")
...
const geoPath = d3
  .geoPath()
  .projection(d3.geoEquirectangular().scale(1000).translate([-1650, 110]));

// Setup color scale
var choroplethLegendText = [
  "%",
  "60",
  "65",
  "70",
...
// Setup container SVG
const choroplethSvg = d3.select("#choropleth").attr("viewBox", "0 0 1200 460");

const mapContainer = choroplethSvg.append("g").classed("map", true);

const mapLegendContainer = choroplethSvg
  .append("g")
  .classed("map-legend", true)
  .attr("transform", "translate(10, 300)");

const sliderContainer = choroplethSvg
  .append("g")
  .attr("transform", "translate(20, 380)");

const pickerContainer = choroplethSvg
  .append("g")
  .attr("transform", "translate(140, 360)");

const distributionContainer = choroplethSvg
  .append("g")
...
Promise.all([d3.csv("data/choropleth.csv"), d3.json("data/indonesia.json")])
...
function updateMapAndDistribution(year, educationLevel, provinces, dataByYear) {
  ...
  yScale.domain([
    d3.min(dataByYear["$" + year], function (d) {
      return d["APM_" + educationLevel];
    }),
    d3.max(dataByYear["$" + year], function (d) {
      return d["APM_" + educationLevel];
    }),
  ]);

  circles = distributionSvg
    .selectAll(".dot")
    ...
    .attr("r", 7)
    .attr("cy", function (d) {
      return yScale(d["APM_" + educationLevel]);
    })
    .style("fill", function (d) {
      return color(d["APM_" + educationLevel]);
    })
    .on("mouseover", function (d) {
      distributionTooltip.transition().duration(200).style("opacity", 0.9);
      distributionTooltip
        .html(d.province + "<br>" + d["APM_" + educationLevel] + "%")
        .style("left", d3.event.pageX + 5 + "px")
        .style("top", d3.event.pageY - 28 + "px");
    })
    .on("mouseout", function (d) {
      distributionTooltip.transition().duration(500).style("opacity", 0);
    })
    .merge(circles)
    .transition();
}
